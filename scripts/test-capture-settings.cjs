...
const settings = vm.runInNewContext(`(() => { ${source}; return { LIVE_CAPTURE_FPS, LIVE_CAPTURE_MAX_DURATION_MS, LIVE_CAPTURE_MAX_FRAMES, LIVE_CAPTURE_VIDEO_BITRATE, LIVE_CAPTURE_LOOP_SEARCH_SECONDS_DEFAULT, LIVE_CAPTURE_LOOP_SEARCH_SECONDS_MAX, normalizeLiveCaptureLoopSearchSeconds }; })()`);
...
assert.equal(settings.LIVE_CAPTURE_LOOP_SEARCH_SECONDS_MAX, 25);
...
assert.equal(settings.normalizeLiveCaptureLoopSearchSeconds(25), 25);
assert.equal(settings.normalizeLiveCaptureLoopSearchSeconds(26), 25);
assert.equal(settings.normalizeLiveCaptureLoopSearchSeconds(9999), 25);
